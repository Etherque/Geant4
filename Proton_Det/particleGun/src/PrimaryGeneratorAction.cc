...
{    
  ...
  ifstream fileinput;
  fileinput.open("./acceptedevents.txt");
  for (int j=0;j<totalevents;j++){
      for (int i=0; i<lists;i++){
           fileinput>>acceptevent[j][i];
          }      
      }
  //InitTheta();
  //InitPsi();
...
{  
  ...
  theta= acceptevent[numofevents][1];
  psi= acceptevent[numofevents][2];
  energy=acceptevent[numofevents][3];
  ...
  // G4double radiusunit = 1.0*mm, lengthunit = 1.0*cm;
  pParticleRadius = acceptevent[numofevents][5]*mm;
  pParticleZ = acceptevent[numofevents][6]*cm;
  pParticleAngle = acceptevent[numofevents][7];
