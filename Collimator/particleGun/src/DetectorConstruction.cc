...
// -------    Material definitions ------------- //
   ...
   G4Element* eT = new G4Element("Triton", "T", z=1, a = 3.000*g/mole);  
   ...
   G4Material* Tritium = new G4Material("Tritium", density = 0.000267*g/cm3, ncomponents = 1);
   Tritium->AddElement(eT, fractionmass = 1.00);
   ...
   BC404->AddElement(C,fractionmass=0.085);
   BC404->AddElement(H,fractionmass=0.915);
...
// ----------- Construct the wire chamber solid and logical volumes. Wire chambers are nearly
   G4double frontDis = 2.45 *cm;//front face to the center of target
   G4double colimLen = 2.5 *cm;//collimator outer extent
   G4double colimHei = 15.0 *mm;//height
   G4double colimZ = 10.0 *cm;//
   G4double colimT = 0.44 *mm;//Thickness of cell wall
   G4double colimN = 3;//number of cells vertical
   
   G4double diameter = (colimHei - colimN * colimT)/colimN;
   
   G4double distance = colimHei / colimN;
   ...
   G4double rIn[] = { 0, 0};
   G4double rOut[] =  {diameter/2, diameter/2};
   G4double zPlane[] = {-colimLen/2 - 0.1*mm, colimLen/2 + 0.1*mm};
   G4VSolid* solidHex = new G4Polyhedra("HexTub", 
                                          0.*deg, 
                                              360.*deg,
                                              6,
                                              2,
   ...
   G4VSolid* subtractL = new G4SubtractionSolid("Box-Hex",
                                                  solidColimator,      
                                                  solidHex,
                                                  rotationMatrix,
   ...
   for(G4int y = -Ymax; y <= Ymax; y++) {
       ...
       //for(G4int z = -1; z <= 1; z++) {
              subtractL = new G4SubtractionSolid("Box-Hex",
                                                  subtractL, 
                                                  solidHex,
                                                  rotationMatrix,
                                                  G4ThreeVector(0 *cm, (y - (z%2)*0.5)*distance, z*distance*0.866));
              if(y == Ymax && z%2 == 1) {
                 subtractL = new G4SubtractionSolid("Box-Hex",
                                                  subtractL, 
                                                  solidHex,
                                                  rotationMatrix,
                                                  G4ThreeVector(0 *cm, (Ymax + 1 -(z%2)*0.5)*distance, z*distance*0.866)); 
              }
      }   
   }
   ...
   fWireChamber = new G4PVPlacement(0,
                                    G4ThreeVector(-(frontDis + colimLen/2), 0*diameter, 0*diameter),
                                    logicMergeL,
                                    "Colimators",
                                     logicWorld,
                                     false,
   ...
   G4VSolid* subtract = new G4SubtractionSolid("Box-Hex",
                                                  solidColimator,      
                                                  solidHex,
                                                  rotationMatrix2,
                                                  G4ThreeVector(0 *cm, 0*diameter, 0*diameter));
   ...
   for(G4int y = -Ymax; y <= Ymax; y++) {
       for(G4int z = -Zmax; z <= Zmax; z++) {
      //for(G4int z = -1; z <=1; z++) {
              subtract = new G4SubtractionSolid("Box-Hex",
                                                  subtract, 
                                                  solidHex,
                                                  rotationMatrix2,
                                                  G4ThreeVector(0 *cm, (y - (z%2)*0.5)*distance, z*distance*0.866)); 
   ...
   logicMergeL->SetVisAttributes(BoxVisAttWhite);
   
  //-------------------------------- detectors --------------------------//
   G4double PlasPos = frontDis + 0.55*cm + colimLen;   

   G4double fdetX = 0.1 *cm;
   G4double plasX = 0.035*mm;
   G4double fdetY = colimHei;
   G4double fdetZ = colimZ;
   G4double fdetPos = PlasPos + 0.483*cm;
   
   G4Box* plastDet = new G4Box("Plastic", 
                              plasX/2, 
                              fdetY/2, 
                              fdetZ/2);
  
   G4LogicalVolume* logPlastic = new G4LogicalVolume(plastDet, 
                                                    BC404, 
                                                    "logPlastic", 
                                                    0,0,0);//Helium
   
   fPlastic =  new G4PVPlacement(0,
	                        G4ThreeVector(PlasPos, 0, 0*cm),
                   		logPlastic,
			        "Plastic",
				logicWorld,
				false,
				0);
  // logPlasticBehind 
  G4LogicalVolume* logPlasticBehind = new G4LogicalVolume(plastDet, 
                                                    Helium, 
                                                    "logPlasticBehind", 
                                                    0,0,0);//Helium
   
   fPlasticBehind =  new G4PVPlacement(0,
	                        G4ThreeVector(PlasPos + 1.1*plasX, 0, 0*cm),
                   		logPlasticBehind,
			        "Behind",
				logicWorld,
				false,
   ...
   logPlastic-> SetVisAttributes(BoxVisAttCyan);
   logPlasticBehind -> SetVisAttributes(BoxVisAttCyan);
   ...
   for(G4double origin = 0.0*cm; origin <= 0.0*cm; origin += 10.1*cm) {
   // ---------- Construct the Havar Tube and make it Cyan ------------------//
   G4double HavarTubeThickness = 0.00635*mm, HavarTubeRadius = 6.5*mm, HavarTubeLength = 10.0*cm/2,
            startAngle = 0.*deg, spanningAngle = 360.*deg;
   G4double HavarTubePosX = 0, HavarTubePosY = 0, HavarTubePosZ = 0 - origin;
   
   G4Tubs* TritonTube = new G4Tubs("TritonTube",
                                   0,
   ...
   G4LogicalVolume* logic3HTube = new G4LogicalVolume(TritonTube,
                                                         Tritium,
                                                         "TritiumTube",
                                                         0,0,0);

   f3HTube = new G4PVPlacement(0,
                                 ...
                                 "TritonTube",
                                  ...
                                  false,
                                  0);
   G4VisAttributes* BoxVisAttRed = new G4VisAttributes(G4Colour(1.0, 0.0, 1.0));
   BoxVisAttRed->SetForceSolid(TRUE);

   logic3HTube->SetVisAttributes(BoxVisAttRed); 
